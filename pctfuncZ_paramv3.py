...
class PowNet(nn.Module):
    def __init__(self, inputdim):
        ...
        # Third layer with even more units
        ...
        # Additional linear layers to align dimensions of skip connections
        self.skip_fc1 = nn.Linear(128, 64)  # Align fc1 with fc2
        self.skip_fc2 = nn.Linear(64, 32)   # Align fc2 with fc3
    ...
    def forward(self, x):
        ...
        # Apply skip connection (x1 is transformed to match x2's shape)
        x1_skip = self.skip_fc1(x1)  # Transform x1 to shape (batch_size, 64)
        x2 += x1_skip  # Skip connection
        
        # Third hidden layer with skip connection
        x3 = F.leaky_relu(self.fc3(x2), negative_slope=0.01)
        x3 = self.dropout(x3)
        
        # Apply skip connection (x2 is transformed to match x3's shape)
        x2_skip = self.skip_fc2(x2)  # Transform x2 to shape (batch_size, 32)
        x3 += x2_skip  # Skip connection
        
        # Fourth layer without activation
        x4 = self.fc4(x3)
        
        # Apply ReLU to ensure no negative values
        x = F.relu(x)
        
        # Rescale to range [1, 6]
        x = x * (6 - 1) / (x.max() - x.min()) + 1  # Rescale between 1 and 6
