...
class PCTKernelLayer(nn.Module):
    ...
    def R_nl(self,r, n, l):
        ...
        for s in range((n - abs(l)) // 2 + 1):
            term = ((-1)**s *
                    ...
                    self.comb(torch.tensor((n + abs(l)) // 2), torch.tensor(s)) *
                    self.comb(torch.tensor((n - abs(l)) // 2), torch.tensor(s)) *
                    r**(n - 2 * s))
            result += term
        return result
    ...
    def R_nl_pzms(self,r, n, l):
        ...
        for s in range((n - abs(l)) // 2 + 1):
            term = ((-1)**s *
                    self.comb(torch.tensor(2 * n + 1 - s), torch.tensor(s)) *
                    self.comb(torch.tensor(n + abs(l)), torch.tensor(s)) *
                    r**(n - s))
            result += term
        return result
    ...
    def Q_n(self,r, n):
        ...
        for s in range(n + 1):
            alpha_ns = ((-1)**(n + s) * 
                        ...
                        (self.factorial_tensor(n - s) * self.factorial_tensor(s) * self.factorial_tensor(s + 1)))
            term = alpha_ns * r**s
            result += term
        return result
