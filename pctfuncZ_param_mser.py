...
class PCT(nn.Module):
    ...
    def keypoint_func(self,img1,img2):
        ...
        # Detect keypoints using MSER
        regions1, _ = mser.detectRegions(gray1)
        regions2, _ = mser.detectRegions(gray2)
        
        # Convert regions to keypoints
        keypoints1 = [cv2.KeyPoint(float(x), float(y), 1) for region in regions1 for (x, y) in region]
        keypoints2 = [cv2.KeyPoint(float(x), float(y), 1) for region in regions2 for (x, y) in region]
        
        # Compute descriptors using BRIEF or another descriptor if needed
